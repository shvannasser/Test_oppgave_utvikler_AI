...
async function fetchYRForecast() {
  const url =
    "https://api.met.no/weatherapi/locationforecast/2.0/compact?lat=59.1313&lon=11.3871"; // Halden
  ...
  try {
    const response = await fetch(url, { headers });
    if (!response.ok) {
      throw new Error(`Failed to fetch YR data: ${response.status}`);
    }
    return await response.json();
  } catch (err) {
    console.error(err);
...
server.tool(
  "get-forecast",
  "Get weather forecast for Halden, Norway",
  {},
  async () => {
    const data = await fetchYRForecast();
    ...
    const nextHour = data.properties.timeseries[0];
    const details = nextHour.data.instant.details;

    const output = [
      `🌤 Weather forecast for Halden (next hour):`,
      `Temperature: ${details.air_temperature}°C`,
      `Wind: ${details.wind_speed} m/s`,
      `Humidity: ${details.relative_humidity}%`,
    ].join("\n");
...
async function main() {
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error("YR Weather MCP Server running...");
}
